...
init( void )
{
    ...
    */
    ...
    //make circle1
    for(GLfloat i = 0.0; i < rad360; i= i + rad6) {
	points[pointCount] = vec2(( h2+(radius*cos(i)) ), ( k2+(radius*sin(i) )));
        colors[pointCount] = base_colors[1];
	pointCount++;
    }
    ...
    //make circle1
    for(GLfloat i = 0.0; i < rad360; i= i + rad6) {
	points[pointCount] = vec2(( h3+(radius*cos(i)) ), ( k3+(radius*sin(i) )));
        colors[pointCount] = base_colors[1];
	pointCount++;
    }
    ...
    GLfloat k4 = -0.2;
    //make circle1
    for(GLfloat i = rad360/2; i < rad360; i= i + rad6) {
	points[pointCount] = vec2(( h4+(radius*cos(i)) ), ( k4+(radius*sin(i) )));
        colors[pointCount] = base_colors[1];
	pointCount++;
    }
    /* 
...
/*
    ...
    glDrawArrays( GL_TRIANGLE_FAN, 85, 60 );    // draw the points
 */
    glDrawArrays( GL_TRIANGLE_FAN, 0, 60 );    // draw the points
    glDrawArrays( GL_TRIANGLE_FAN, 61, 60 );    // draw the points
    glDrawArrays( GL_TRIANGLE_FAN, 122, 60 );    // draw the points
    glDrawArrays( GL_TRIANGLE_FAN, 183, 30 );    // draw the points
    glFlush();
}
